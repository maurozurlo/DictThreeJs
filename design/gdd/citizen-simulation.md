...
| **Coup** (`CoupHandler`, ADR-0009) | indirect | The coup reads effective `peopleRelation`. Protestor feedback lowering it pushes *toward* an overthrow loss (−10) while pulling *away* from a people-coup (which arms at high relation). This tension is intended; see Edge Cases. |
...
| `bodySeed` | per-ped constant in [0,1), set at creation | fixed |
...
Two independent, permanent paths (dead peds are removed for the run, never replaced):

1. **gone** — a ped whose role resolves to `gone` (§4.3) leaves the board.
2. **starvation** — civilian-dependent peds only (people-faction, plus *displaced* army/business; employed elites are fed by their institution and immune):
...
| Education fork cut | ≤4 thief / ≥5 protestor | 3–6 split | Where ignorance becomes dissent. The core "too dumb to revolt" lever. |
