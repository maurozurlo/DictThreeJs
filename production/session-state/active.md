...
Feature: 9-1/9-2/9-3/9-4 complete (incl. round-1 opening + camera bug fix)
Task: Should Have 9-5 (crisis docket check) ready-for-dev, not started
<!-- /STATUS -->
...
## Session Extract — Camera bug fix + Story 9-4 (round-1 opening) 2026-07-08 (same day, follow-up)
- User playtested and reported: after a round ends, the camera stays at the last-used tab's position (e.g. Meet) instead of showing Street — only fixes itself once the player manually clicks the Street tab. Root cause: `expireTimer()` force-set `tabs.activeTab: Tabs.Street` directly via a raw `set()`, bypassing the camera-positioning logic that only lived inside the `setActiveTab()` action (position/fov/hFov/rotation are computed there per-tab, not derived from `activeTab` as a side effect).
- Fix: extracted the Street camera position/fov/hFov/rotation into a shared `STREET_CAMERA` constant in `src/Constants/GameState.ts` (previously inlined only in `setActiveTab`'s Street branch). Applied it explicitly in both `expireTimer()` branches (adds a `scene.camera` patch alongside the `tabs.activeTab` write) and in `StateFactory.buildStartState` (see below). `setActiveTab`'s Street branch now just references the constant too — single source of truth.
- Same message, user independently proposed exactly Story 9-4 (scoped but unimplemented from the same session): "start on street view, then move into decision-making" — i.e. see the city → act → see the city again, rather than the existing act → see city → act. Implemented it: new games now open on `Tabs.Street` with `dwelling: true` and `timerStartedAt: null` (timer paused) via `buildStartState`; a new store action `beginFirstWorkDay()` (distinct from `nextRound()` — no round to resolve for the very first transition) starts the timer and moves to `Tabs.Log` when the player dismisses the intro. `DayEnded.tsx` branches on `isIntro = !dayEnded` to show a one-time `hinge.intro_headline` ("A NEW ERA BEGINS") + "Begin Month 1" button instead of the round-recap flow — same mandatory-reveal/dwell component as Story 9-3, no new UI built.
- Files touched: src/Constants/GameState.ts (+STREET_CAMERA, +MANDATORY_REVEAL_MS already existed), src/Stores/GameState.ts (expireTimer camera patch ×2, new beginFirstWorkDay action, setActiveTab references shared constant), src/Stores/StateFactory.ts (buildStartState opens on Street/dwelling/paused timer/camera), src/types/GameState.ts (+beginFirstWorkDay), src/components/DayEnded/DayEnded.tsx (isIntro branch, guard changed from `!dayEnded` to `!dwelling`), locales EN/ES (+hinge.intro_headline, +actionPanel.begin_first_month).
- Test regressions from changing the DayEnded guard (`!dayEnded`→`!dwelling`) and buildStartState's new default: fixed 2 pre-existing tests that assumed dayEnded alone gates the modal (store_selector_render.test.tsx) and that a fresh store opens away from Street (tab_gating.test.ts) — both were testing an invariant that's now different by design, not real regressions.
- New tests: tests/unit/roundloop/intro_dwell.test.ts (8 tests, includes a direct regression guard for the camera bug: corrupt scene.camera, call expireTimer(), assert it snaps back to STREET_CAMERA).
- Verified end-to-end via Puppeteer against the live dev server: new game → screenshot shows Street scene with "A NEW ERA BEGINS" headline (not a locked decision screen); navigated to Meet tab; forced round-end; screenshot confirms camera correctly shows Street (not stuck on Meet) — direct visual proof the bug is fixed.
- Suite 704/704 passing, tsc -b clean. Committed and pushed (same autonomous-sprint cadence).
- Next: design doc §8's actual validation question ("does the player linger or mash?") requires real human playtesting across 3-4 months — not something this session can answer. Should Have 9-5 (crisis docket consistency check) still scoped and ready whenever picked up.

## Session Extract — Code Audit & Simplification Plan 2026-07-08 (same day)
- TRIGGER: user feels the game "supposed to be fairly simple" has grown convoluted; asked for a plan covering complexity reduction, dead code, superseded logic, SRP, ADR compliance. Asked questions BEFORE writing (per request).
- User decisions via AskUserQuestion: (1) audit doc + cleanup sprint, review before executing; (2) gameplay-preserving but SAVES MAY BREAK; (3) only truly dead code — live features stay; (4) ADR conflicts arbitrated case by case.
- DELIVERABLE WRITTEN: `docs/architecture/code-audit-2026-07-08.md` — full findings + proposed Sprint 10 (7 stories, 10-1..10-7). NOT committed; awaiting user review. Sprint 10 files NOT created yet.
- Headline findings: GameState.ts (1,276 lines) is the problem — nextRound() 5 near-duplicate set() blobs, expireTimer() twin branches, setActiveTab() 3 concerns, ~290 lines of inline logic belonging in Handlers (weird-law path, periodic/miniChallenge resolve near-duplicates of each other, shop.buy). swapLaw uses an OLDER law-picker than RoundResolver.pickNextLaw (ignores rep statuses — possible live bug). Dead: LanguageSwitcher.tsx, getRandomDailyEventForPower, middleground/ scratch folder (git-tracked 3ds Max dumps), ~10 unused exports + ~30 unused type exports (knip). visualConsequences.ts is DORMANT scaffolding for visual update — keep. 16 test files in src/ violate "tests live in tests/" standard. ADR-0007 status stale ("Proposed, Sprint 5") — recommend Superseded-by-0008. Components/hooks verified CLEAN on SRP; no three-imports in UI; handlers don't import store; Math.random uses all cosmetic/seed (allowed).
- Knip false positives documented in the audit doc (global.d.ts, .claude hook, tools/ipl, prototypes, balance_calc) — do not delete.
- NEXT STEP: user reviews audit doc → if approved, create sprint-10.md + story files and execute story-by-story (10-1 nextRound de-dup is the big one).
